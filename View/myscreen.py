...
class MyScreenView(MDScreen):
    ...
    def close_dialog(self, dialog_data: list = []):
        if self.dialog.mode == "input":
            self.controller.input_sportsman(dialog_data)
        elif self.dialog.mode == "filter":
            self.controller.filter_sportsmans(dialog_data)
        elif self.dialog.mode == "delete":
            unlucky = self.controller.delete_sportsmans(dialog_data)
            Snackbar(text=f"{unlucky} sportsmen were deleted!").open()
